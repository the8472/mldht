...
public class Node {
	...
	private Map<Integer,Key> usedIDs = new HashMap<Integer, Key>();
	...
	void recieved (DHTBase dh_table, MessageBase msg) {
		...
		for (RoutingTableEntry e : routingTable)
			nodeIDchange |= e.bucket.checkForIDChangeAndNotifyOfResponse(msg);
	...
	private void updateEntryCount() {
		int newNumEntries = 0;
		for (RoutingTableEntry e : routingTable)
			newNumEntries += e.bucket.getNumEntries();
	...
	public void insertEntry (KBucketEntry entry, boolean internalInsert) {
		...
		while(tableEntry.bucket.getNumEntries() >= DHTConstants.MAX_ENTRIES_PER_BUCKET)
		{
			boolean isPrefixOfAny = false;
			for(Key k : allLocalIDs())
				isPrefixOfAny |= tableEntry.prefix.isPrefixOf(k);
			if(!isPrefixOfAny)
				break;
		...
		if(internalInsert)
			tableEntry.bucket.sortedInsert(entry);
		else
			tableEntry.bucket.insert(entry);
	}
	...
	public Key getDerivedID(int idx) {
		if(usedIDs.containsKey(idx))
			return usedIDs.get(idx);
		Key id = getRootID().getDerivedKey(idx);
		usedIDs.put(idx, id);
		return id;
	}
	
	public Collection<Key> allLocalIDs() {
		return usedIDs.values();
	}
	...
	*/
	void onTimeout (MessageBase msg) {
		...
		if(survivalMode)
			return;
		for(RoutingTableEntry e : routingTable)
			e.bucket.onTimeout(msg.getDestination());
	}
